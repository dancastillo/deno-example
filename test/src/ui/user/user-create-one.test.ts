...
import {
  CREATE_USER,
  createOneUserInDatabaseForTesting,
  dropCreateOneUserInDatabaseForTesting,
...
Deno.test("WorkspaceService :: Routes :: GET one user", async (t) => {
  // Load initial mock data for testing Create One User
  await createOneUserInDatabaseForTesting();

  await t.step("Create One User", async () => {
    ...
    assertEquals(result, { user: CREATE_USER });

    abortController.abort();
    await fastify.close();
  });

  // Drop mock data for testing Create One User
